...
using JsonConverter.Newtonsoft.Json.Utils;
using Stef.Validation;

namespace JsonConverter.Newtonsoft.Json.Dynamic;

internal static class DynamicJsonClassFactory
{
    ...
    private const string DynamicAssemblyName = "JsonConverter.Dynamic, Version=1.0.0.0";
    private const string DynamicModuleName = "JsonConverter.Dynamic";

    static DynamicJsonClassFactory()
    {
    ...
    internal static DynamicJsonClass CreateInstance(IList<DynamicPropertyWithValue> dynamicPropertiesWithValue, bool createParameterCtor = true)
    {
        var type = CreateType(dynamicPropertiesWithValue, createParameterCtor);
        var dynamicClass = (DynamicJsonClass)Activator.CreateInstance(type)!;
    ...
    internal static Type CreateType(IList<DynamicPropertyWithValue> properties, bool createParameterCtor = true)
    {
        ...
        if (!GeneratedTypes.TryGetValue(key, out var type))
        {
            ...
            lock (GeneratedTypes)
            {
                if (!GeneratedTypes.TryGetValue(key, out type))
                {
                    ...
                    TypeBuilder tb = ModuleBuilder.DefineType(name, TypeAttributes.AnsiClass | TypeAttributes.Public | TypeAttributes.Class | TypeAttributes.AutoLayout | TypeAttributes.BeforeFieldInit, typeof(DynamicJsonClass));
                    ...
                    type = GeneratedTypes.GetOrAdd(key, tb.CreateType()!);
                }
            }
        }
    ...
    /// <returns></returns>
    private static string GenerateKey(IEnumerable<DynamicPropertyWithValue> dynamicProperties, bool createParameterCtor)
    {
